...
export default function WorkerDashboard() {
  ...
  return (
    <div className="flex flex-col gap-5 h-full w-full pb-10">
      ...
      <div className="flex items-center gap-3 bg-white border border-gray-200 rounded-xl p-4 text-base font-medium text-farm-text shadow-sm">
      ...
      <div className="grid grid-cols-4 gap-5">
        ...
        <div className="col-span-2 bg-[#3B4D36] rounded-xl p-6 text-white shadow-sm flex flex-col justify-between min-h-[160px]">
          <div className="flex items-center gap-4 mb-2">
            ...
            <div>
              ...
              <p className="text-sm text-white/80 font-medium">Overview This month</p>
            </div>
          </div>
          <div className="text-[48px] font-extrabold leading-none tracking-tight">{formatBaht(totalBalanceThisMonth)} <span className="text-3xl font-bold ml-1 tracking-normal">THB</span></div>
        </div>
        ...
        <div className="col-span-1 bg-white rounded-xl p-6 shadow-sm border border-gray-100 flex flex-col justify-between min-h-[160px]">
          <div>
            <h2 className="text-xl font-bold text-farm-text">Today's income</h2>
            <p className="text-sm text-gray-400 font-medium mt-1">Overview today</p>
          </div>
          <div className="text-[42px] font-extrabold text-[#3B4D36] leading-none tracking-tight">{formatBaht(todayIncome)} <span className="text-2xl font-bold ml-1 tracking-normal">THB</span></div>
        </div>
        ...
        <div className="col-span-1 bg-white rounded-xl p-6 shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center min-h-[160px]">
          <h2 className="text-lg font-bold text-gray-400 mb-3">สถานะการจ่ายเงิน</h2>
          <div className="bg-[#D1A344]/30 text-[#91712A] px-6 py-2.5 rounded-full font-bold flex items-center gap-2 text-lg mb-3">
            <div className="border-[2.5px] border-[#91712A] rounded-full p-0.5">
              <ArrowDownToLine size={16} strokeWidth={3} />
            </div>
            รอจ่าย
          </div>
          <p className="text-gray-400 font-medium text-[13px]">รอบจ่ายถัดไป: 31 สิงหาคม 2569</p>
        </div>
      </div>

      <div className="mt-2">
        <h2 className="text-3xl font-extrabold text-farm-text">Total Work Done</h2>
      </div>
        ...
        {statsBoxes.map((item, idx) => {
          ...
          return (
            <div key={idx} className="bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col">
              <div className="bg-[#FFFDF4] px-4 py-3 rounded-lg flex items-center gap-3 m-3 border border-[#F3EFE6]">
                <Icon size={24} strokeWidth={2.5} className="text-[#D1A344]" />
                <span className="font-extrabold text-farm-text text-lg">{item.title}</span>
              </div>
              
              <div className="px-6 pb-6 text-left mt-2">
                <div className="flex items-baseline gap-2 mb-2">
                  <span className="text-4xl font-extrabold text-farm-text leading-none">{item.amountNum}</span>
                  <span className="text-[15px] font-bold text-gray-400">{item.unit}</span>
                </div>
                <div className="flex items-baseline gap-1.5">
                  <span className="text-[22px] font-extrabold text-farm-text leading-none">{item.thbStr}</span>
                  <span className="text-[13px] font-bold text-gray-400">บาท</span>
                </div>
              </div>
            </div>
          );
      ...
      <div className="grid grid-cols-3 gap-5 h-[360px]">
        ...
        <div className="bg-white rounded-xl p-7 shadow-sm border border-gray-100 flex flex-col h-full">
          <h3 className="text-farm-text font-extrabold text-xl mb-4">Total Work Done</h3>
          <div className="flex-1 w-full h-full min-h-0">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartBars} margin={{ top: 20, right: 0, left: -25, bottom: 0 }} barSize={38}>
                <XAxis dataKey="label" axisLine={false} tickLine={false} tick={{ fontSize: 13, fill: '#9ca3af', fontWeight: 'bold' }} dy={10} />
                <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 13, fill: '#9ca3af', fontWeight: 'bold' }} domain={[0, 40]} ticks={[0, 5, 10, 15, 20, 25, 30, 35, 40]} />
                ...
                <Bar dataKey="value" fill="#E5E7EB" activeBar={{ fill: '#2B3E26' }} radius={[4, 4, 0, 0]}>
                  ...
                  <LabelList dataKey="value" position="top" style={{ fill: '#374151', fontSize: 14, fontWeight: '900' }} dy={-6} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
        ...
        {/* Donut Chart */}
        <div className="bg-white rounded-xl p-7 shadow-sm border border-gray-100 flex flex-col h-full">
          <h3 className="text-farm-text font-extrabold text-xl mb-2 w-full text-left">Tasks by Month</h3>
          ...
          <div className="relative w-full h-[160px] flex items-center justify-center shrink-0 mt-2">
            ...
            <div className="absolute inset-0 m-auto flex flex-col items-center justify-center pointer-events-none mt-2">
              <span className="text-sm font-bold text-farm-text">รวม</span>
              <span className="text-[22px] font-extrabold text-farm-text leading-tight mt-1">{formatBaht(totalBalanceThisMonth)}</span>
              <span className="text-sm font-bold text-farm-text mt-0.5">บาท</span>
            </div>
          </div>
          ...
          <div className="w-full grid grid-cols-2 gap-x-4 gap-y-5 mt-8">
            {statsBoxes.map((stat, idx) => (
              <div key={stat.id} className="flex items-center gap-3">
                <div className="w-2 h-[48px] rounded-full shrink-0" style={{ backgroundColor: donutColors[idx] }}></div>
                <div className="flex-1 flex flex-col justify-center">
                  <span className="text-sm font-bold text-gray-800 leading-tight">{stat.title}</span>
                  <span className="text-xs text-gray-400 font-bold leading-tight mt-1">{stat.amountNum} {stat.unit}</span>
                  <span className="text-xs text-gray-400 font-bold leading-tight mt-0.5">{stat.thbStr} บาท</span>
                </div>
                <div className="text-3xl font-extrabold text-farm-text shrink-0">{stat.pct}%</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-7 shadow-sm border border-gray-100 flex flex-col h-full">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-farm-text font-extrabold text-xl">Recent Activity</h3>
            <Link to="/worker/history" className="text-base font-bold text-farm-text hover:text-[#708238] flex items-center gap-1">
              ...
              return (
                <div key={item.id} className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-base font-extrabold text-farm-text">
                        {WORK_LOG_TYPES[item.type].labelTh} <span className="text-gray-400 font-bold mx-1">·</span> <span className="font-bold text-gray-500 text-sm">{WORK_LOG_TYPES[item.type].summaryText(item)}</span>
                      </p>
                      <p className="text-[13px] font-bold text-gray-400 mt-1">{formatDate(item.date)}</p>
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-lg font-extrabold text-[#537626]">+{formatBaht(item.total)}</p>
                  </div>
                </div>
              );
            })}
