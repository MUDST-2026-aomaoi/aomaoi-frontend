...
export default function WorkerDashboard() {
  ...
  return (
    <div className="flex flex-col gap-4 h-full max-w-[1200px] w-full pb-8">
      ...
      {/* 2. Top Cards (Grid 4 คอลัมน์ ให้ My balance กินพื้นที่ 2 คอลัมน์ตาม Figma) */}
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="col-span-2 bg-[#3B4D36] rounded-xl p-6 text-white shadow-sm flex flex-col justify-between min-h-[140px]">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-xs text-white/80 font-medium">Overview This month</p>
            </div>
          </div>
          <div className="text-4xl font-extrabold">{formatBaht(totalBalanceThisMonth)} <span className="text-2xl font-bold">THB</span></div>
        </div>
        ...
        <div className="col-span-1 bg-white rounded-xl p-6 shadow-sm border border-gray-100 flex flex-col justify-between min-h-[140px]">
          <div>
            ...
            <p className="text-xs text-gray-400 font-medium mt-0.5">Overview today</p>
          </div>
          <div className="text-4xl font-extrabold text-[#3B4D36] tracking-tight">{formatBaht(todayIncome)} <span className="text-2xl font-bold">THB</span></div>
        </div>
        ...
        <div className="col-span-1 bg-white rounded-xl p-6 shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center min-h-[140px]">
          <h2 className="text-base font-bold text-gray-400 mb-3">สถานะการจ่ายเงิน</h2>
          <div className="bg-[#D1A344]/30 text-[#91712A] px-6 py-2 rounded-full font-bold flex items-center gap-2 text-base mb-3">
      ...
      <div className="mt-2">
        <h2 className="text-xl font-extrabold text-farm-text">Total Work Done</h2>
      </div>
        ...
        {statsBoxes.map((item, idx) => {
          ...
          return (
            <div key={idx} className="bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col">
              {/* ป้ายสีเหลืองชิดซ้ายบน มี margin เล็กน้อย */}
              ...
              <div className="px-5 pb-5 text-left mt-1">
                ...
                <div className="flex items-baseline gap-2">
                  <span className="text-lg font-extrabold text-farm-text leading-none">{item.thbStr}</span>
      ...
      <div className="grid grid-cols-3 gap-4 h-[320px]">
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 flex flex-col h-full">
          <h3 className="text-farm-text font-extrabold text-lg mb-4">Total Work Done</h3>
          <div className="flex-1 w-full h-full min-h-0">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartBars} margin={{ top: 20, right: 0, left: -25, bottom: 0 }} barSize={32}>
                ...
                <Bar dataKey="value" fill="#E5E7EB" activeBar={{ fill: '#2B3E26' }} radius={[4, 4, 0, 0]}>
                  ...
                  <LabelList dataKey="value" position="top" style={{ fill: '#374151', fontSize: 13, fontWeight: '900' }} dy={-8} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
        ...
        {/* Donut Chart */}
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 flex flex-col h-full">
          <h3 className="text-farm-text font-extrabold text-lg mb-2 w-full text-left">Tasks by Month</h3>
          ...
          <div className="relative w-full h-[140px] flex items-center justify-center shrink-0">
            ...
            <div className="absolute inset-0 m-auto flex flex-col items-center justify-center pointer-events-none mt-2">
              ...
              <span className="text-[17px] font-extrabold text-farm-text leading-tight">{formatBaht(totalBalanceThisMonth)}</span>
              <span className="text-[11px] font-bold text-farm-text">บาท</span>
            </div>
          </div>

          {/* Legend 2x2 ตรงตามดีไซน์เป๊ะ (เส้นสีซ้ายสุด ตัวหนังสือ % ใหญ่ขวาสุด) */}
          <div className="w-full grid grid-cols-2 gap-x-4 gap-y-4 mt-6">
            {statsBoxes.map((stat, idx) => (
              <div key={stat.id} className="flex items-center gap-3">
                {/* แถบสีแนวตั้ง */}
                <div className="w-1.5 h-10 rounded-full shrink-0" style={{ backgroundColor: donutColors[idx] }}></div>
                
                {/* ข้อมูล 3 บรรทัด */}
                <div className="flex-1 flex flex-col">
                  <span className="text-[11px] font-bold text-gray-800">{stat.title}</span>
                  <span className="text-[9px] text-gray-400 font-medium">{stat.amountNum} {stat.unit}</span>
                  <span className="text-[9px] text-gray-400 font-medium">{stat.thbStr} บาท</span>
                </div>
                
                {/* เปอร์เซ็นต์ตัวใหญ่ */}
                <div className="text-lg font-extrabold text-farm-text shrink-0">{stat.pct}%</div>
              </div>
            ))}
          </div>
        </div>
              ...
              return (
                <div key={item.id} className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-[13px] font-extrabold text-farm-text">
                        {WORK_LOG_TYPES[item.type].labelTh} · <span className="font-bold text-gray-500">{WORK_LOG_TYPES[item.type].summaryText(item)}</span>
                      </p>
                      <p className="text-[11px] font-bold text-gray-400 mt-0.5">{formatDate(item.date)}</p>
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-sm font-extrabold text-[#537626]">+{formatBaht(item.total)}</p>
                  </div>
                </div>
              );
            })}
